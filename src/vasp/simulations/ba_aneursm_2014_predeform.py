...
def set_problem_parameters(default_variables, **namespace):
    ...
    default_variables.update(
        dict(
            ...
            theta=1.0,  # Theta scheme (implicit/explicit time stepping)
            lmbda=0.5,
            save_step=10,  # Save frequency of files for visualisation
            checkpoint_step=10,  # Save frequency of checkpoint files
            ...
            id_in=(2, 3),  # inlet id for the fluid
            ...
            v_max_final_lva=2.58,  # Final max centerline velocity of parabolic profile
            v_max_final_rva=2.28,  # Final max centerline velocity of parabolic profile
            # should be the cycle-averaged average velocity for your main simulation
            P_final=10932.404,  # Steady State pressure applied to wall
            ...
            robin=True,  # Use Robin BC for solid displacement
...
def create_bcs(DVP, mesh, boundaries, t_start_v, t_end_v, t_start_p, t_end_p, P_final,
               v_max_final_lva, v_max_final_rva, fsi_id, id_in, inlet_outlet_s_id, rigid_id, psi, F_solid_linear, **namespace):
    ...
    # Fluid velocity BCs
    dsi = ds(id_lva, domain=mesh, subdomain_data=boundaries)
    n = FacetNormal(mesh)
    ...
    u_inflow_exp_lva = VelInPara(t=0.0, t_start=t_start_v, t_end=t_end_v, v_max_final=v_max_final_lva,
                             n=normal, dsi=dsi, mesh=mesh, degree=3)
    u_inlet_lva = DirichletBC(DVP.sub(1), u_inflow_exp_lva, boundaries, id_lva)

    dsi = ds(id_rva, domain=mesh, subdomain_data=boundaries)
    n = FacetNormal(mesh)
    ndim = mesh.geometry().dim()
    ni = np.array([assemble(n[i] * dsi) for i in range(ndim)])
    n_len = np.sqrt(sum([ni[i] ** 2 for i in range(ndim)]))  # Should always be 1!?
    ...
    u_inflow_exp_rva = VelInPara(t=0.0, t_start=t_start_v, t_end=t_end_v, v_max_final=v_max_final_rva,
                             n=normal, dsi=dsi, mesh=mesh, degree=3)
    u_inlet_rva = DirichletBC(DVP.sub(1), u_inflow_exp_rva, boundaries, id_lva)
    ...
    bcs = [u_inlet_lva, u_inlet_rva, u_inlet_s, d_inlet_s]

    return dict(bcs=bcs, u_inflow_exp_lva=u_inflow_exp_lva, u_inflow_exp_rva=u_inflow_exp_rva, p_out_bc_val=p_out_bc_val,
                F_solid_linear=F_solid_linear)


def pre_solve(t, u_inflow_exp_lva, u_inflow_exp_rva, p_out_bc_val, **namespace):
    # Update the time variable used for the inlet boundary condition
    u_inflow_exp_rva.update(t)
    u_inflow_exp_lva.update(t)
    p_out_bc_val.update(t)
    return dict(u_inflow_exp_rva=u_inflow_exp_rva, u_inflow_exp_lva=u_inflow_exp_lva, p_out_bc_val=p_out_bc_val)
